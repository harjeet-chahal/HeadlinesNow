...
useEffect(()=>{
    const getUser=async()=> {
        const URL=`
        https://newsapi.org/v2/top-headlines?country=in&category=sports&apiKey=8054cbf260c34c07b069dc7f331ec4bd`
      try {
        const response = await axios.get(URL);
        console.log("this is response",response.data.articles);
        setSportss(response.data.articles)
